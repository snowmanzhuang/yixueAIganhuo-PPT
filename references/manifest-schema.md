...
`editppt run finalize` writes two final decks and reports both paths in `final/run_summary.json`:

- `editable_output`: final editable PPTX assembled from recorded page manifests.
- `image_only_output`: final image-only PPTX assembled from the generated/source slide images.

Both outputs reuse `notes_manifest.json` speaker notes when notes are available.
...
`editppt run finalize` consumes this file for both the final editable PPTX and final image-only PPTX.
