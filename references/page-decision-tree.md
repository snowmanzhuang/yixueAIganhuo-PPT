...
The order exists because steps 1-2 decide object sources and step 3 consumes those decisions. Nativizing text and layout first locks in wrong choices: text that belongs to a logo, a UI screenshot, or a to-be-separated asset must not become a native text box, and which text needs clean-base removal depends on the background decision. Define the boundaries between background, foreground, and native structure first; then write the manifest. This order costs no wall-clock time — start the PaddleOCR/text-hints pass immediately, submit all step-1/2 image jobs as one `editppt image batch` call, and fill text boxes from the measured hints while those jobs run.

PaddleOCR/text-hints pass immediately means the page worker verifies existing `text_hints.json`/`text_hints.png` or runs `editppt page hints <page_dir>` as soon as the mandatory references are read. This text-hints work runs in parallel with foreground asset separation where tooling allows. Do not wait for foreground image jobs to finish before starting OCR, and do not consume final text placement before object-source decisions are recorded.
...
### 2.0 Source Material Original Priority

When a foreground evidence object clearly comes from a source material original file supplied to the full-authoring workflow, prefer the original material file as the asset source. This applies to paper Figures, clinical images, chart screenshots, microscopy/radiology images, or other evidence panels that the upstream plan bound to a slide.

This preference is not a direct crop shortcut. Use the source material original as:

- the `editppt image edit --image <original-material>` input for asset-sheet separation; or
- an evidence-image placement source when the original Figure is intended to remain a rectangular evidence image.

Record the decision in `visual_inventory` and `asset_provenance`. The provenance note must state whether the object used asset-sheet separation or evidence-image placement, and must name the source material original. Do not silently crop from generated `source.png` when the corresponding original evidence file is available.
...
Whoever rebuilds the page checks it once against this list — deterministic validation is necessary but not sufficient, and the parent agent does not repeat this check. Record the evidence in structured manifest fields and `validation.json`. Deck-level structural QA happens when the parent runs `editppt run finalize`.
